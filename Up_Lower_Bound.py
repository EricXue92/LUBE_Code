...
class UpperLowerBound:
    ...
    def build_model(self):
        ...
        curr = layers.Dense(64, activation='relu', kernel_regularizer=regularizers.l2(0.0001), 
        kernel_initializer='normal')(inputs) 
        curr = layers.Dropout(self.drop_out)(curr)

        curr = layers.Dense(32, activation='relu', kernel_regularizer=regularizers.l2(0.0001),
        kernel_initializer='normal')(curr)
        curr = layers.Dropout(self.drop_out)(curr)

        # Lower bound  (head)
        lower_bound = layers.Dense(8, kernel_regularizer=regularizers.l2(0.0001), activation = 'linear')(curr)
        if self.flag:
            lower_bound = layers.Dropout(self.drop_out)(lower_bound)
        lower_bound = layers.Dense(1, bias_initializer = keras.initializers.constant(-1.0),
        name = 'lower_bound')(lower_bound)

        # Upper bound  (head)
        upper_bound = layers.Dense(8, kernel_regularizer=regularizers.l2(0.0001), activation = 'linear')(curr)  
        if self.flag:
    ...
    # Transform scaled data to original data
